...
<!-- REVIEW(developer): With 1 km² regions, a 5 km boss radius fans ghost-sync across a ~5-region radius exactly at peak concurrency, and "isolate the hotspot to one server" doesn't reduce that fan-out — publish a worst-case world-boss sync budget (neighbouring servers holding the ghost, snapshot volume per tick) or move the radii/region size. -->
