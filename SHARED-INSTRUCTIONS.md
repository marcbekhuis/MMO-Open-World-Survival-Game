...
Creature detail files follow a recognizable shape — opening behaviour and role prose, then a `Story Hook` section, the `See also:` backlink, an optional `Concept Drawing` section when art exists, and finally the `Draft` appendix — which new creatures should mirror. Keep concept art embeds in that separate `## Concept Drawing` section rather than inline near the opening paragraph. Keep the prose in the systems-design voice of the surrounding document throughout, framing decisions in terms of gameplay rationale, and link any file you name as you write so the web of documents stays navigable.
